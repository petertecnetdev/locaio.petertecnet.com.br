...
const state = {
  ...
  useType: 'all',
  sort: 'newest',
  view: localStorage.getItem('locaio_property_view') || 'grid',
};
...
const propertySearchText = (property) => normalize([
  ...
  USE_LABELS[property.use_type],
].filter(Boolean).join(' '));

const findPropertiesPage = () => {
  const pages = [...document.querySelectorAll('.pt-page')];
  return pages.find((page) => normalize(page.querySelector('.pt-page-header h1')?.textContent) === 'imoveis') || null;
};
...
const refreshProperties = async () => {
  ...
  try {
    ...
    // A tela React continua funcional mesmo se o enriquecimento visual falhar.
  } finally {
    state.loading = false;
  }
};
...
const summaryHtml = () => {
  ...
  return `
    <section class="pm-property-summary" aria-label="Resumo da carteira de imóveis">
...
const controlsHtml = () => `
  <section class="pm-property-toolbar" aria-label="Filtros de imóveis">
    <div class="pm-property-search">
      <span aria-hidden="true">⌕</span>
      <input type="search" value="${escapeHtml(state.query)}" placeholder="Buscar por imóvel, rua, bairro ou cidade" aria-label="Buscar imóveis" data-pm-search />
    </div>
    <div class="pm-property-filters">
      <select data-pm-status aria-label="Filtrar por status">
      ...
      <select data-pm-use aria-label="Filtrar por finalidade">
        <option value="all">Todas as finalidades</option>
        <option value="residential">Residencial</option>
        <option value="commercial">Comercial</option>
        <option value="mixed">Misto</option>
      </select>
      <select data-pm-sort aria-label="Ordenar imóveis">
        <option value="newest">Mais recentes</option>
        <option value="name">Nome A–Z</option>
        <option value="rent_desc">Maior aluguel</option>
        <option value="rent_asc">Menor aluguel</option>
      </select>
      <div class="pm-view-toggle" role="group" aria-label="Modo de visualização">
        <button type="button" data-pm-view="grid" aria-label="Visualização em cartões">▦</button>
        <button type="button" data-pm-view="list" aria-label="Visualização em lista">☷</button>
      </div>
    </div>
...
const bindToolbar = (host) => {
  ...
  useType?.addEventListener('change', (event) => {
    state.useType = event.currentTarget.value;
    applyFiltersAndDecorations();
  });
...
const decorateCard = (card, property) => {
  ...
  if (status) status.textContent = STATUS_LABELS[property.status] || property.status || 'Disponível';

  const address = card.querySelector('.pt-property-body > div:first-child p');
  if (address) address.innerHTML = `<span aria-hidden="true">⌖</span>${escapeHtml(propertyAddress(property) || 'Endereço não informado')}`;
  ...
  if (details) {
    ...
    if (use) use.textContent = `${TYPE_LABELS[property.type] || 'Imóvel'} · ${USE_LABELS[property.use_type] || 'Uso não definido'}`;
    if (rent) rent.textContent = property.default_rent_amount ? money(property.default_rent_amount) : 'Valor a definir';
  }
  ...
  features.innerHTML = propertyFeatureHtml(property);
  ...
  if (leaseButton) {
    ...
    leaseButton.title = canLease ? 'Criar uma nova locação para este imóvel' : `Imóvel ${normalize(STATUS_LABELS[property.status] || property.status)} — altere o status antes de criar uma locação.`;
  }
};
...
const applyFiltersAndDecorations = () => {
  ...
  cards.forEach((card, index) => {
    ...
    } else if (card.dataset.propertyId) {
      byId.set(String(card.dataset.propertyId), card);
    }
  });
  ...
  state.properties.forEach((property) => {
    ...
    if (!card) return;
    card.hidden = !visibleIds.has(String(property.id));
  });
...
const openPropertyModal = (property = null) => {
  ...
  backdrop.innerHTML = `
    <section class="pm-property-modal" role="dialog" aria-modal="true" aria-labelledby="pm-property-modal-title">
      <header>
        <div><span>Patrimônio</span><h2 id="pm-property-modal-title">${editing ? 'Editar imóvel' : 'Cadastrar imóvel'}</h2><p>${editing ? 'Atualize cadastro, disponibilidade e valores de referência.' : 'Cadastre o imóvel com informações suficientes para contratos e gestão.'}</p></div>
        <button type="button" class="pm-modal-close" aria-label="Fechar" data-pm-close>×</button>
      </header>
      <form class="pm-property-form">
        <div class="pm-form-error" data-pm-form-error hidden></div>
        ...
        <section><h3>Endereço</h3><div class="pm-form-grid three">
          ...
          ${input('Número', 'number', current.number)}
          ${input('Complemento', 'complement', current.complement)}
          ${input('Bairro', 'neighborhood', current.neighborhood)}
          ${input('Cidade', 'city', current.city, { required: true })}
          ${input('UF', 'state', current.state, { required: true, maxLength: 2 })}
        </div></section>
  ...
  form.addEventListener('submit', async (event) => {
    ...
    const payload = {
      ...
      parking_spaces: optionalNumber(data.parking_spaces),
      area_m2: optionalNumber(data.area_m2),
      default_rent_amount: optionalNumber(data.default_rent_amount),
      default_due_day: optionalNumber(data.default_due_day) || 10,
    };
    submit.disabled = true;
    submit.textContent = editing ? 'Salvando…' : 'Cadastrando…';
    errorBox.hidden = true;
    try {
      if (editing) await appApi.patch(`/properties/${property.id}`, payload);
      else await appApi.post('/properties', payload);
      close();
...
const interceptCreateButtons = (event) => {
  ...
  if (button.closest('.pm-property-modal')) return;
...
async function enhancePropertiesPage() {
  ...
  state.page = page;
  if (pageChanged || !state.properties.length) await refreshProperties();
  renderManagementHost();
  applyFiltersAndDecorations();
}
