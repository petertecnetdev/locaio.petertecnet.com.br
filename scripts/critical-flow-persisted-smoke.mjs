...
const state = {
  property: { id: 101, name: 'Apartamento E2E', type: 'apartment', use_type: 'residential', status: 'occupied', street: 'Rua de Teste', number: '100', city: 'Goiânia', state: 'GO', default_rent_amount: 2200, default_due_day: 10 },
  lease: { id: 501, public_id: 'lease-e2e-501', property_id: 101, property_name: 'Apartamento E2E', landlord_user_id: 9000001, tenant_user_id: 9000002, tenant_name: 'Inquilino E2E', tenant_email: 'inquilino.e2e@example.invalid', tenant_tax_id: '12345678909', purpose: 'residential', status: 'draft', starts_on: '2026-08-01', ends_on: '2027-07-31', rent_amount: 2200, due_day: 10, deposit_months: 1, deposit_amount: 2200, guarantee_type: 'deposit', adjustment_index: 'IPCA', included_expenses: [], tenant_expenses: ['iptu', 'water', 'electricity'], is_in_force: false, metadata: {} },
...
const respond = (request, payload = {}, status = 200) => request.respond({ status, headers, body: status === 204 ? '' : JSON.stringify(payload) });
...
function makeContract(status = 'review') {
  const content = 'CONTRATO DE LOCAÇÃO RESIDENCIAL\n\nLocador: Locador E2E\nLocatário: Inquilino E2E\nAluguel mensal: R$ 2.200,00.';
  return {
    id: 601, public_id: 'contract-e2e-601', title: 'Contrato de locação — Apartamento E2E', status, current_version: 1,
    parties: [{ id: 701, role: 'landlord', email: fakeUser.email }, { id: 702, role: 'tenant', email: state.lease.tenant_email }],
    signatures: [],
    versions: [{ id: 611, version: 1, status, content, content_hash: 'abcdef1234567890abcdef1234567890', is_locked: !['review', 'draft'].includes(status), created_at: now() }],
  };
}
...
async function mockApi(request) {
  ...
  if (method === 'OPTIONS') return respond(request, {}, 204);
  ...
  if (method === 'POST' && path.endsWith('/leases/501/proposal/generate')) return respond(request, { document: { id: 590, title: 'Proposta de locação — Apartamento E2E', status: 'review', current_version: 1, versions: [{ id: 591, version: 1, status: 'review', content: 'PROPOSTA DE LOCAÇÃO\nAluguel: R$ 2.200,00.' }] } }, 201);
  if (method === 'POST' && path.endsWith('/leases/501/contract/generate')) { state.contract = makeContract(); state.lease.contract_text = state.contract.versions[0].content; return respond(request, { document: clone(state.contract) }, 201); }
  ...
  return respond(request, { message: `Escrita inesperada bloqueada: ${method} ${path}` }, 418);
}
...
async function reloadLease(page) {
  await page.reload({ waitUntil: 'domcontentloaded' });
  await openLease(page);
}

async function assertOwnerActions(page) {
  ...
  if (['Pix', 'Boleto', 'Cartão'].some((label) => visible.some((text) => text.includes(label)))) throw new Error(`A visão do proprietário expõe checkout do inquilino: ${visible.join(', ')}`);
  if (!visible.some((text) => text.includes('Marcar recebido'))) throw new Error('A ação Marcar recebido não está disponível para o proprietário.');
}
...
try {
  ...
  page.on('request', (request) => {
    ...
    if (url.hostname === 'api.petertecnet.com.br') { mockApi(request).catch((error) => { console.error('[mock]', error); if (!request.isInterceptResolutionHandled?.()) request.abort(); }); return; }
    request.continue();
  });

  await page.evaluateOnNewDocument((user) => {
    localStorage.setItem('token', 'locaio-critical-persisted-e2e');
  ...
  if (!response || response.status() >= 400) throw new Error(`Locaio respondeu HTTP ${response?.status() ?? 'sem resposta'}.`);
  ...
  await page.waitForFunction(() => true, { timeout: 150 });
  if (!state.calls.some((call) => call.endsWith('/contract/generate'))) throw new Error('A geração do contrato não foi solicitada.');

  await reloadLease(page);
  ...
  if (!confirmations.keys || !confirmations.final) throw new Error('Confirmações do distrato não foram encontradas.');
  ...
  if (state.calls.some((call) => call.endsWith('/charges/901/payment'))) throw new Error('O proprietário tentou iniciar checkout do inquilino.');
  for (const suffix of ['/proposal/generate', '/contract/generate', '/contract/send', '/contract/sign', '/charges/schedule', '/charges/901/paid', '/termination/complete']) {
    if (!state.calls.some((call) => call.endsWith(suffix))) throw new Error(`Etapa crítica não exercitada: ${suffix}`);
  }

  console.log('E2E persistido aprovado: proposta → contrato → assinatura → cobrança → baixa → distrato, sem escrita na produção.');
} finally {
  await browser.close();
}
